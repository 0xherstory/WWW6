...
const CharityDrawer = ({ open, onOpenChange, charityPool = '0', totalVolume = '0' }: CharityDrawerProps) => {
  const { toast } = useToast();
  
  // 格式化 ETH 數據
  const formattedCharityPool = parseFloat(charityPool).toFixed(4);
  const formattedTotalVolume = parseFloat(totalVolume).toFixed(4);
        ...
        <div className="space-y-6">
          ...
          <div className="gradient-earth rounded-2xl p-6 text-center shadow-medium">
            <div className="flex items-center justify-center gap-2 text-primary-foreground/80 mb-2">
              ...
              <span className="text-sm font-medium">公益资金池 (鏈上數據)</span>
            </div>
            <p className="text-4xl font-serif font-bold text-primary-foreground">
              {formattedCharityPool} ETH
            </p>
            <p className="text-sm text-primary-foreground/70 mt-1">
              總投注量: {formattedTotalVolume} ETH
            </p>
          </div>
